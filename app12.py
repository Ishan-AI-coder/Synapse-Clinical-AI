...
with t5:
    ...
    st.caption("Upload physical PDF/Image reports for auto-extraction and visual trend analysis via Gemini AI.")
    
    import os
    import streamlit.components.v1 as components
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    html_path = os.path.join(current_dir, "report_ui.html")
    
    if os.path.exists(html_path):
        with open(html_path, "r", encoding="utf-8") as f:
            html_content = f.read()
            
        # Securely inject the API URL and the User's JWT Token into the Javascript
        html_content = html_content.replace("{{API_URL}}", API_URL)
        html_content = html_content.replace("{{TOKEN}}", st.session_state.token)
            
        components.html(html_content, height=1200, scrolling=True)
    else:
        st.error(f"🚨 Missing File Error: The system cannot find 'report_ui.html'.")